...
// Project Imports - Ensure these match your actual file paths
...
class _StoryModeScreenState extends State<StoryModeScreen> {
  ...
  void _markPageAsKnown(String pageContent) {
    ...
    for (var word in words) {
      ...
      // Only mark if not currently in vocab or status is 0 (New)
            ...
            itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
              ...
              PopupMenuItem<String>(
                ...
                child: Row(
                  children: [
                    Icon(
                      _isSavedToLibrary ? Icons.bookmark : Icons.bookmark_border,
                      color: _isSavedToLibrary ? theme.primaryColor : Colors.grey,
                    ),
                    const SizedBox(width: 8),
                    Text(_isSavedToLibrary ? 'Saved to Library' : 'Save to Library'),
                  ],
                ),
              ),
            ...
            onPressed: () {
              if (_isTtsPlaying) {
                ...
                if (_currentPage < _storyPages.length) {
                  ...
                  _flutterTts.speak(widget.lesson.sentences.join(". "));
                }
              }
            },
          ...
          child: ElevatedButton(
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Launching Quiz...")),
              );
            },
  ...
  void _handleMenuAction(String value) {
    if (value == 'toggle_mark_swipe') {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_autoMarkOnSwipe 
            ? "Swipe will now mark pages as known" 
            : "Swipe marking disabled"
          ),
  ...
  void _toggleSaveToLibrary() {
    ...
    final updatedLesson = widget.lesson.copyWith(
      isFavorite: _isSavedToLibrary,
      userId: user.id,
    );
    context.read<LessonBloc>().add(LessonUpdateRequested(updatedLesson));
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(_isSavedToLibrary ? "Saved to your Library" : "Removed from Library"),
        duration: const Duration(seconds: 1),
      ),
    );
  }
  ...
  Widget _buildStoryPage(String content, bool isDark) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _buildInteractiveText(content, isDark),
  ...
  Widget _buildKeyPhrasesPage(bool isDark) {
    return ListView(
      ...
      children: [
        ...
        Text(
          "Tap to listen, Long press to translate",
          ...
          return Card(
            ...
            child: ListTile(
              ...
              leading: CircleAvatar(
                ...
                child: Icon(Icons.volume_up, color: Theme.of(context).primaryColor, size: 20),
              ),
              title: Text(
                sentence,
                style: TextStyle(
  ...
  void _showLimitDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Free translation limit reached. Upgrade to Premium for unlimited access."),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text("Cancel")),
          ElevatedButton(
            onPressed: () {
              Navigator.pop(context);
              showDialog(context: context, builder: (_) => const PremiumLockDialog());
            },
