...
class _StoryModeScreenState extends State<StoryModeScreen> {
  ...
  Widget _buildInteractiveText(String content, bool isDark) {
    ...
    for (int i = 0; i < words.length; i++) {
      ...
      if (vocabItem != null && vocabItem.status >= 1 && vocabItem.status <= 4) {
        ...
        highlightColor = Colors.blue.withValues(alpha: .05);
      }
        ...
        child: SafeArea(
          child: ElevatedButton(
            onPressed: () => ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text("Launching Quiz..."))),
  ...
  void _toggleSaveToLibrary() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          _isSavedToLibrary ? "Saved to your Library" : "Removed from Library",
        ),
        duration: const Duration(seconds: 1),
      ),
    );
  ...
  Widget _buildKeyPhrasesPage(bool isDark) {
    return ListView(
      ...
      children: [
        ...
        if (widget.lesson.sentences.isEmpty)
          const Padding(
            padding: EdgeInsets.all(16.0),
            child: Text("No key phrases available."),
          ),
  ...
  void _showDefinitionDialog(
    ...
    final translationFuture =
        (existingItem != null
            ...
            .then((value) => value ?? 'No translation available');
    // final geminiPrompt =
    //     "Translate and explain: '$originalText' (Language: ${widget.lesson.language} -> ${user.nativeLanguage}).";
  ...
  void _showLimitDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "Free translation limit reached. Upgrade to Premium for unlimited access.",
        ),
        actions: [
          TextButton(
