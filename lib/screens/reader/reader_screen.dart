...
//   // TODO: Double check this key is valid and has Gemini API enabled in Google Cloud Console
...
//     super.initState();
//     try {
//       // Re-init is safe in most cases, or check if already initialized if needed
...
//     if (_isSentenceMode) {
//       // If the video has moved to a new sentence (because user pressed play),
//       // update the UI immediately. This allows "normal play".
...
//         // Auto-Pause ONLY if:
//         // 1. We are playing
//         // 2. We hit the exact end of the *currently displayed* sentence
//         // 3. We haven't already moved past it (buffer check)
//         if (_isPlaying && 
//             currentSeconds >= activeLine.end && 
//             currentSeconds < activeLine.end + 0.5) {
//            // We only pause if the video hasn't transitioned to the next sentence yet
//            // If the user hit play, realTimeIndex might already be next, avoiding this pause.
...
//       // 1. Try Translation Service (Robust Catch)
//       try {
//         standardTranslation = await translationService.translate(
//           originalText, 
//           user.nativeLanguage, 
//           widget.lesson.language
//         );
//       } catch (e) {
//         standardTranslation = "Translation unavailable";
...
//         })
//         .catchError((e) {
//           geminiTranslation = "Gemini Error (403 or other): $e. Check API Key.";
//         });
...
class _ReaderScreenState extends State<ReaderScreen> {
  // fallback key (only used if dotenv isn't configured). Remove in production.
  final String _fallbackGeminiApiKey =
      "";
  ...
  void initState() {
    ...
    // Load Gemini API key from .env (flutter_dotenv should be loaded in main())
    try {
      final envKey = dotenv.env['GEMINI_API_KEY'];
      final keyToUse = (envKey != null && envKey.isNotEmpty)
          ? envKey
          : _fallbackGeminiApiKey;
      Gemini.init(apiKey: keyToUse);
    } catch (e) {
      print("Gemini Init Error: $e");
    }
  ...
  // Improved hit-testing: use global bounds of each word's RenderBox
  void _handleDragUpdate(int sentenceIndex, Offset globalPosition) {
    final keys = _wordKeys[sentenceIndex];
    if (keys == null) return;
    ...
    for (int i = 0; i < keys.length; i++) {
      ...
      final renderBox = context.findRenderObject() as RenderBox?;
      if (renderBox != null) {
        // get global bounding rect
        final topLeft = renderBox.localToGlobal(Offset.zero);
        final size = renderBox.size;
        final rect = topLeft & size;

        // allow a small inflate so selection is forgiving
        final inflated = rect.inflate(6.0);

        if (inflated.contains(globalPosition)) {
          if (_selectionEndIndex != i) {
            setState(() {
              _selectionEndIndex = i;
            });
  ...
  void _finishSelection(String fullSentence) {
    ...
    // Use same splitting as UI so indices match
    final words = fullSentence.split(RegExp(r'(\s+)'));
  ...
  Widget _buildSentence(String sentence, int sentenceIndex) {
    ...
    return Wrap(
      ...
      children: words.asMap().entries.map((entry) {
        ...
        if (cleanWord.isEmpty || word.trim().isEmpty) {
          // whitespace or punctuation tokens: still provide a small hit area container
          return Container(
            key: wordKey,
  ...
  void _showCombinedDialog(
    ...
    // 2. Call Gemini and await result BEFORE showing modal so UI updates immediately
    try {
      final resp = await Gemini.instance.prompt(parts: [Part.text(geminiPrompt)]);
      geminiTranslation = resp?.output ?? "No output from Gemini";
    } catch (e) {
      geminiTranslation = "Gemini Error (403 or other): $e. Check API Key.";
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) {
          // the translations are ready already (we awaited), but keep the small auto-refresh fallback
          if (standardTranslation == 'Loading...' ||
              geminiTranslation == 'Loading...') {
            Future.delayed(Duration(milliseconds: 500), () {
              if (context.mounted) setModalState(() {});
