...
//   // --- CONSTANTS ---
//   static const int _kFreeLookupLimit = 50; // Number of words allowed
//   static const int _kResetMinutes = 10; // Minutes before counter resets
...
//                   onPlayPressed: _isVideo ? () {} : _toggleTtsFullLesson,
//                 ),
...
import 'package:linguaflow/models/lesson_model.dart';
import 'package:linguaflow/models/vocabulary_item.dart';
// NOTE: Replaced ReaderTopBar with standard AppBar to implement menu requirements
import 'package:linguaflow/services/translation_service.dart';
import 'package:linguaflow/services/vocabulary_service.dart';
import 'package:linguaflow/widgets/translation_sheet.dart';
import 'package:linguaflow/widgets/premium_lock_dialog.dart';
...
class _ReaderScreenState extends State<ReaderScreen> {
  ...
  // --- SETTINGS ---
  bool _autoMarkOnSwipe = false; // New Setting
  ...
  Color _getWordColor(VocabularyItem? item) {
    if (item == null || item.status == 0) return Colors.blue.withOpacity(0.15); // Unknown word base
    switch (item.status) {
  ...
  void _nextSentence() {
    if (_activeSentenceIndex < _smartChunks.length - 1) {
      // Logic for marking previous word as known
      _handleSwipeMarking(_activeSentenceIndex);
      setState(() {
        _activeSentenceIndex++;
        _currentSentenceTranslation = null;
  ...
  void _handleSwipeMarking(int leavingIndex) {
    ...
    for (var word in words) {
      ...
      // Mark as Known (Status 5) if it's currently Unknown (Status 0 or null)
      if (item == null || item.status == 0) {
        _updateWordStatus(clean, word.trim(), "", 5, showDialog: false);
        markedAny = true;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: bgColor,
      // --- REPLACED ReaderTopBar WITH STANDARD APPBAR FOR MENU SUPPORT ---
      appBar: AppBar(
        elevation: 0,
        backgroundColor: bgColor,
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              if (value == 'toggle_mark_swipe') {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(_autoMarkOnSwipe ? "Mark known on swipe enabled" : "Mark known on swipe disabled"),
            ...
            itemBuilder: (context) => [
              PopupMenuItem<String>(
                ...
                child: Row(
                  children: [
                    Icon(
                      _autoMarkOnSwipe ? Icons.check_box : Icons.check_box_outline_blank,
                      color: _autoMarkOnSwipe ? theme.primaryColor : Colors.grey,
                    ),
                    const SizedBox(width: 8),
        ...
        // Add ProgressBar to bottom of AppBar
      ...
      body: SafeArea(
        child: Stack(
          children: [
            Column(
              children: [
                // (Optional) Slider logic could go here if not in AppBar
                if (_isVideo) _buildVideoHeader(isDark),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: GestureDetector(
              onLongPress: () => _showLingQDialog("sentence_$index", text, isPhrase: true),
              child: _buildSentence(text, index),
            ),
          ),
        ],
      ...
      children: words.asMap().entries.map((entry) {
          ...
          return Container(
            ...
            child: Text(
              ...
              style: TextStyle(
                fontSize: fontSize,
                height: lineHeight,
                color: isOverlay ? Colors.white : (isDark ? Colors.white : Colors.black87),
              ),
            ),
          );
        }
        ...
        if (isOverlay) {
          ...
          if (bgColor != Colors.transparent && bgColor != Colors.blue.withOpacity(0.15)) {
           ...
           // If word has a highlight (other than the base "unknown" blue tint)
  ...
  void _showLingQDialog(String cleanId, String originalText, {required bool isPhrase}) {
    ...
    if (existingItem != null)
      ...
      translationFuture = translationService.translate(originalText, user.nativeLanguage, widget.lesson.language).catchError((e) => "Translation unavailable");
      ...
      showDialog(
        ...
        builder: (ctx) => Center(
          child: Material(
            color: Colors.transparent,
            child: Container(
              width: 350, height: 250, padding: EdgeInsets.zero,
              decoration: BoxDecoration(color: const Color(0xFF1E1E1E), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white24, width: 1)),
              child: ClipRRect(
                borderRadius: BorderRadius.circular(16),
                child: TranslationSheet(
                  originalText: originalText, translationFuture: translationFuture, geminiFuture: geminiFuture,
                  isPhrase: isPhrase, existingItem: existingItem, targetLanguage: widget.lesson.language, nativeLanguage: user.nativeLanguage,
                  onSpeak: () => _flutterTts.speak(originalText),
