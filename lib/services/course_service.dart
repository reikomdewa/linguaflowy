...
class CourseService {
  ...
  Future<List<LessonModel>> getCourseLessons({
    ...
    try {
      ...
      final allLessons = jsonList.map((j) => LessonModel.fromMap(j, j['id'])).toList();
      ...
      if (userLevel.toLowerCase().contains('intermediate')) userDifficulty = 'intermediate';
      if (userLevel.toLowerCase().contains('advanced')) userDifficulty = 'advanced';
