...
class _LiveNotificationBannerState extends State<LiveNotificationBanner>
  ...
  void _pickSmartRoom() {
    ...
    List<ChatRoom> candidates = _currentAvailableRooms.where((r) {
      ...
      // --- DEBUG PRINT PER ROOM ---
      String status = "Valid";
      bool keep = true;

      // Check Active
      if (r.isActive != true) {
        status = "REJECTED (Inactive: ${r.isActive})";
        keep = false;
      } 
      // Check Own Room
      else if (currentUid != null && r.hostId == currentUid) {
        status = "REJECTED (Own Room)";
        keep = false;
      }
